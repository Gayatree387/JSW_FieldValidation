...
public class SpecialChar extends BaseTest implements JSW_TestData
{
	...
	@Test(priority = 2)
	public void CkeckSpecialCharForSAPProductCode()throws InterruptedException
	{
		JSW_MasterModule masterdata = new JSW_MasterModule(driver);
		Thread.sleep(700);
		masterdata.SAPProductCodeSpecialCharMethod();
		Thread.sleep(700);
		...
		if (constainsSpecial == true) {
			AutoConstant.fileone.setCellData(Product_Sheet, "Status", 10, "Fail");
			System.out.println("Fail");
		} 
		else {
			AutoConstant.fileone.setCellData(Product_Sheet, "Status", 10, "Pass");
